...
function plot_results(objective_history, regularization_history, gradient_norm_history, objectives; scale=log10)
    f = Figure()
    ax = Axis(f[1, 1], xlabel="Iteration", ylabel="Objective ($(objectives.interior_objective) + $(objectives.regularization))", yscale=scale)
    gradient_ax = Axis(f[1, 1], ylabel="Gradient Norm", yaxisposition=:right, yscale=scale, ygridstyle=:dot)
